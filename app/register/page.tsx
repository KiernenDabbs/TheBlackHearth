      ...
      <div className="min-h-screen bg-background flex items-center justify-center px-6">
        <div className="text-center space-y-6 max-w-md">
          <div className="relative mx-auto w-20 h-20">
            ...
            <div className="relative w-20 h-20 rounded-full bg-card border border-violet-500/40 flex items-center justify-center">
              <Flame className="h-8 w-8 text-violet-400" />
            </div>
          </div>
          <h2 className="text-3xl font-[family-name:var(--font-one-slice)] text-foreground">Check Your Email</h2>
          <p className="text-muted-foreground leading-relaxed">
            We sent a confirmation link to <span className="text-foreground">{email}</span>. 
            Click it to activate your account and enter the Hearth.
          </p>
          <Link href="/login" className="text-violet-400 hover:text-violet-300 transition-colors text-sm">
            Back to Sign In →
          </Link>
        </div>
      </div>
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <p className="text-muted-foreground leading-relaxed">
                Join a fellowship of dark fantasy enthusiasts. Your Vault awaits.
              </p>
            </div>
            <div className="space-y-3">
              {["Track your tabletop collection", "Build wishlists for upcoming releases", "Access exclusive campaign settings", "Connect with fellow adventurers"].map((f) => (
          ...
          <p className="text-muted-foreground text-sm">
            Already a member?{" "}
            <Link href="/login" className="text-violet-400 hover:text-violet-300 transition-colors">Sign in here</Link>
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-violet-400 text-sm tracking-widest uppercase font-medium">Join the Hearth</span>
            </div>
            <h1 className="text-4xl font-[family-name:var(--font-one-slice)] text-foreground">Create Account</h1>
            <p className="text-muted-foreground">Begin your dark journey. No experience required.</p>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              <Label htmlFor="username" className="text-foreground/80 text-sm tracking-wide">Username</Label>
              <Input id="username" type="text" placeholder="DarkAdventurer42" value={username}
            ...
            <div className="space-y-2">
              <Label htmlFor="confirm" className="text-foreground/80 text-sm tracking-wide">Confirm Password</Label>
              <div className="relative">
                <Input id="confirm" type={showConfirm ? "text" : "password"} placeholder="••••••••" value={confirm}
                  onChange={(e) => setConfirm(e.target.value)}
                  className={`bg-card border-border/60 focus:border-violet-500/60 h-11 text-foreground placeholder:text-muted-foreground pr-10 ${confirm.length > 0 && !rules.match ? "border-red-500/50" : ""} ${rules.match ? "border-violet-500/50" : ""}`} />
                <button type="button" onClick={() => setShowConfirm(!showConfirm)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors">
                  {showConfirm ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
              {confirm.length > 0 && !rules.match && <p className="text-xs text-red-400">Passwords do not match</p>}
            </div>
            ...
            <Button onClick={handleRegister} disabled={!allValid || loading}
              ...
              {loading ? "Forging your account..." : "Forge My Account"}
            </Button>
